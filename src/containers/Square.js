...
import { moveTo, animeMove, attackTo, kamickAttack } from '@/store/actions'
...
const Square = ({y, x, me, partner, canMove, isLight, isRock, isTreasure, isAttacked}) => {
  ...
  if(herePartner && hereMe) {
    ...
    if(first.substr(0,1) === 'D') {
      ...
      //   store.dispatch(kamickAttack({
      //     me: {id, xp: myXpRes, Y:newY, X:newX}, 
      //     partner: {idp, xp:partnerXpRes, Y:Yp, X:Xp},
      //     deadM: null,
      //     deadP: null //checkLogic
      // })) //after anime
        ...
        //оба анигилировались
        store.dispatch(kamickAttack({deadM:{id, Y, X}, deadP: {id:idp, Y:pY, X:pX}, me: null, partner: null}))
        flyUnit = null
        place = null
